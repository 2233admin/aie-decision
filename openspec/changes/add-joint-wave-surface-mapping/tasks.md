...
- [~] 1.5 经人工审核后生成 baseline 并验证 schema 与 engine
  - **RESCINDED — v5 migration attempted but rejected.**  The v5 baseline
    generated from clean 410375b tree showed objective degradation of ALL
    gated metrics vs the existing v4 ratchet: quality 8617 < 8795,
    coupling 67.78 > 60.51, god_files 7 > 6.  The dirty-tree cleanup
    explains the deltas but does not satisfy the non-degradation gate.
    v4 baseline with corrupted provenance (false sourceCommit b300bb43)
    is preserved as the failing fail-closed gate until a non-degraded
    clean-tree anchor can be established.
  - v4 backup: `.sentrux/baseline.v4.corrupted-provenance.backup.json`
    SHA256: 497C0A06E2CC9DD9248B3D2ABF19CF1021D1F596348EE866E1ACDC92F5C325B2
...
- [ ] 1.8 将 `.sentrux/baseline.json` 从 v4 schema 迁移到 v5 schema（`code-intel-sentrux-baseline.v5`）
  - **BLOCKED — baseline metrics divergence.** v4 baseline was saved from a tree
    state (79 files, 478 edges, quality_signal 8795) that does not match the
    `sourceCommit` b300bb43 tree (59 files, 361 edges, quality_signal 8910).
    The v5 baseline generated by compiled sentrux on b300bb43 produces metrics
    differing from the v4 baseline across files, coupling_score, quality_signal,
    god_file_count, functions, cross_module_edges, branch_density_per_fn, and
    max_complexity.  Migration requires identical gated metrics per migration
    protocol; blocked until v4 baseline can be re-anchored from the same tree
    state as sourceCommit.  Do NOT save_baseline from current HEAD.
  - Migration procedure attempted 2026-08-05: checkout b300bb43 in isolated
    worktree, generate v5 baseline with compiled code-intel sentrux, compare
    metrics — divergence confirmed.
  - Adversarial regression tests: `tests/test_v5_godfiles_regression.py` (4
    tests: v5 without godFiles fails, count ratchet enforced, identity
    replacement not masked by same count, valid baseline passes).
