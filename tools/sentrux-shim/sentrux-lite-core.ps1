...
function Resolve-TargetPath {
    ...
    if (-not $item.PSIsContainer) { throw "target path is not a directory: $Path" }
...
function Test-SkippedPath {
    ...
    if ($normalized -match "(^|/)(static|public|wwwroot)/") { return $true }
    $leaf = [System.IO.Path]::GetFileName(($Path -replace "\\", "/"))
    $leafLower = $leaf.ToLowerInvariant()
    if ($leafLower -match "(\.min|\.bundle)\.(js|jsx|mjs|cjs)$") { return $true }
    if ($leaf -match ".+-[A-Za-z0-9_]{6,}\.(js|jsx|mjs|cjs)$" -and $leaf -match "[0-9]" -and $leaf -cmatch "[A-Z]") { return $true }
    if ($leafLower -match "\.[0-9a-f]{12,}(\.rtl)?\.(js|jsx|mjs|cjs|css)$") { return $true }
    $parts = @($normalized -split "/" | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
    foreach ($part in $parts) {
        if ($part.StartsWith(".") -and $part.Length -gt 1 -and $part -notin @(".", "..")) { return $true }
        if ($part -in @(
        ...
        if ($part -match "^(\.venv|venv|env)-") { return $true }
    }
    return $false
}
...
function Get-RelativePathSafe {
    ...
    try { return [System.IO.Path]::GetRelativePath($Base, $Path) }
    catch {
        try {
...
function Measure-File {
    param([string]$TargetPath, [System.IO.FileInfo]$File)
    $text = ""
    try { $text = Get-Content -LiteralPath $File.FullName -Raw -ErrorAction Stop; if ($null -eq $text) { $text = "" } }
    catch { $text = "" }
    ...
    $maxComplexity = 1
    if ($functionCount -gt 0) { $maxComplexity = [Math]::Max(1, [int][Math]::Ceiling($complexityTerms.Count / [Math]::Max(1, $functionCount)) + 1) }
    elseif ($complexityTerms.Count -gt 0) { $maxComplexity = [Math]::Max(1, [int]$complexityTerms.Count + 1) }
    if ($File.Extension.ToLowerInvariant() -eq ".v") {
        $importMatches = @([regex]::Matches($text, "(?m)^\s*import\s+([A-Za-z_][\w.]*)") | Where-Object { $_.Groups[1].Value -notin @("os", "time", "json", "strings", "arrays", "maps", "math") })
        $callMatches = @([regex]::Matches($text, "\b(?:([A-Za-z_]\w*)\.)?([A-Za-z_]\w*)\s*\(") | Where-Object { $_.Groups[1].Value -notin @("os", "time", "json", "strings", "arrays", "maps", "math") })
    } else {
    ...
    return [ordered]@{
        ...
        loc = $loc; functions = $functionCount; max_complexity = $maxComplexity
        imports = $importMatches.Count; calls = $callMatches.Count
...
function Get-SafeSum {
    ...
    if ($null -ne $result -and $null -ne $result.PSObject.Properties['Sum'] -and $null -ne $result.Sum) { return [int]$result.Sum }
    return 0
}
...
function Get-SafeMaximum {
    ...
    if ($null -ne $result -and $null -ne $result.PSObject.Properties['Maximum'] -and $null -ne $result.Maximum) { return [int]$result.Maximum }
    return 0
}
...
function Measure-Project {
    ...
    return [ordered]@{
        tool = "sentrux-lite"
        path = $TargetPath; quality_signal = $quality; files = $fileCount
        import_modeled_files = $modeledFileCount; functions = $functions
        coupling_score = $couplingScore; cycle_count = 0
        god_file_count = $godFiles; complex_fn_count = $complexFiles
        max_complexity = $maxComplexity; total_import_edges = $importEdges
        cross_module_edges = $importEdges; call_edges = $callEdges
        unresolved_imports = 0; files_detail = $fileMetrics
    }
}

# =========================================================================
# Cache-only baseline contract (OpenSpec 1.6).
#
# The ONLY modification from the canonical beta.5 sentrux-lite-core.ps1:
# baseline reads and writes go through .sentrux/cache/lite-baseline.json
# instead of .sentrux/baseline.json.  The native baseline is NEVER read
# or written.  All metric computation is identical to the canonical release.
# =========================================================================

function Get-BaselinePath {
    param([string]$TargetPath)
    return (Join-Path (Join-Path $TargetPath ".sentrux") "cache" "lite-baseline.json")
}

function Write-Baseline {
    param([string]$TargetPath, [object]$Metrics)
    ...
    $payload = [ordered]@{
        ...
        path = $TargetPath; quality_signal = $Metrics.quality_signal
        coupling_score = $Metrics.coupling_score; cycle_count = $Metrics.cycle_count
        god_file_count = $Metrics.god_file_count; complex_fn_count = $Metrics.complex_fn_count
        cross_module_edges = $Metrics.cross_module_edges; total_import_edges = $Metrics.total_import_edges
        files = $Metrics.files; functions = $Metrics.functions
...
function Write-GateOutput {
    param([object]$Before, [object]$After, [bool]$Saved = $false, [string]$BaselinePath = "")
    ...
    if ($Saved) { Write-Output "Baseline saved: $BaselinePath" }
}
...
function Invoke-Health {
    ...
    $target = Resolve-TargetPath $Path
    $metrics = Measure-Project $target
    [ordered]@{ status = "ok"; tool = "sentrux-lite"; quality_signal = $metrics.quality_signal
        files = $metrics.files
...
function Invoke-Check {
    ...
    if (-not (Test-Path -LiteralPath $rulesPath -PathType Leaf)) {
        ...
        $script:SentruxLiteExitCode = 0; return
    }
    ...
    if ($violations.Count -gt 0) {
        ...
        $script:SentruxLiteExitCode = 1; return
    }
...
function Invoke-Gate {
    ...
    foreach ($arg in $GateArgs) {
        if ($arg -eq "--save") { $save = $true } else { $paths.Add($arg) }
    }
    ...
    if ($regressed) { Write-Output "Quality degraded during this session"; exit 1 }
...
switch ($command) {
    "scan" { Invoke-Scan ($(if ($tail.Count -gt 0) { $tail[0] } else { "" })); exit 0 }
    "health" { Invoke-Health ($(if ($tail.Count -gt 0) { $tail[0] } else { "" })); exit 0 }
    "check" { Invoke-Check ($(if ($tail.Count -gt 0) { $tail[0] } else { "" })); exit $script:SentruxLiteExitCode }
    "gate" { Invoke-Gate $tail }
    "plugin" { Write-Output "sentrux-lite: plugin management is not available in lite mode"; exit 0 }
    default { Write-Output "sentrux-lite: unknown command '$command'"; Show-Help; exit 1 }
}
